---
layout: post
title: "앱 9개를 죽이고 배운 것 — '만들 수 있다'와 '만들어야 한다' 사이"
date: 2026-07-07
...
## 테제: 기술은 다 성공했고, 제품은 다 실패했다

FitQuest는 10,150줄에 파일 78개짜리 운동 RPG였다. 완성됐고, 동작했고, 아무 가치가 없었다. 버튼을 누르면 "운동 완료"로 처리되는 게임 — 속이는 대상이 결국 자기 자신이라, 존재 이유가 증발했다.
...
끝장은 끝말잇기 게임이었다(7,346줄, 412K 단어 DB, ~2주). 싱글 5모드, 멀티플레이, AI 대전, 실시간 채팅, 전국 랭킹 — 완성도 95%, 출시 가능한 수준이었다.

그런데 앱인토스 채널 승인이 없었다. 채널 승인 없이는 토스 3,000만 유저에 **접근 자체가 불가능하다.** 핵심 유통 경로가 잠긴 상태에서 완성도 95%를 만든 것이다. 완성도가 높다는 게 계속해야 할 이유는 아니다 — 뭉치뭉치 쪽에 더 구체적인 성과 가능성이 있었고, 기회비용을 계산해 멈췄다.

교훈: **유통 채널부터 열고 개발하라.** 채널이 잠겨 있으면 완성도 100%도 출시 불가다. 그리고 멀티플레이는 닭-달걀 문제다 — 유저가 없으면 실제 대전이 안 되니 기능 가치조차 검증할 수 없다. 싱글로 먼저 검증한 뒤 멀티에 투자했어야 했다.
...
- **이터널리턴 전략도우미** (6,752줄): 게임 루트 편집기. 게임 패치마다 스크래핑 스크립트 30개를 재실행해야 했고 — 데이터 유지보수 부담을 과소평가했다 — dak.gg·er.op.gg 같은 팀 단위 운영 서비스와 1인 개발로 경쟁해야 했다. 게임 도구는 그 게임의 수명에 종속된다.
